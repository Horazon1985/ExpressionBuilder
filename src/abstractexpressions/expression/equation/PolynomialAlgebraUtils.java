...
public abstract class PolynomialAlgebraUtils {
    ...
    public static ExpressionCollection solvePolynomialEquation(ExpressionCollection coefficients, String var) throws EvaluationException {
         ...
         */
        int m = SimplifyPolynomialUtils.getGGTOfAllExponents(coefficients);
        if (m > 1 && degree / m <= ComputationBounds.BOUND_ALGEBRA_MAX_DEGREE_OF_POLYNOMIAL) {
        ...
        m = SimplifyPolynomialUtils.getPeriodOfCoefficients(coefficients);
        if (m < coefficients.getBound()) {
            return PolynomialAlgebraUtils.solvePeriodicPolynomialEquation(coefficients, var);
        }

        // (Nichttriviale) Polynome sollen nur dann exakt gelöst werden, wenn deg - ord <= gewisse Schranke ist.
        if (degree <= ComputationBounds.BOUND_ALGEBRA_MAX_DEGREE_OF_POLYNOMIAL) {
            ...
            if (SimplifyPolynomialUtils.isPolynomialRational(coefficients)) {
                try {
                    Expression factorizedPolynomial = SimplifyPolynomialUtils.decomposePolynomialIntoSquarefreeFactors(coefficients, var);
                    // Fall: Faktorisiertes Polynom ist ein nichttriviales Produkt.
                    if (factorizedPolynomial.isProduct()
                            && SimplifyPolynomialUtils.getDegreeOfPolynomial(((BinaryOperation) factorizedPolynomial).getLeft(), var).compareTo(BigInteger.ZERO) > 0
                            && SimplifyPolynomialUtils.getDegreeOfPolynomial(((BinaryOperation) factorizedPolynomial).getRight(), var).compareTo(BigInteger.ZERO) > 0) {
                        return SimplifyUtilities.union(SolveGeneralEquationUtils.solveZeroEquation(((BinaryOperation) factorizedPolynomial).getLeft(), var),
                                SolveGeneralEquationUtils.solveZeroEquation(((BinaryOperation) factorizedPolynomial).getRight(), var));
                    }
                    ...
                    if (factorizedPolynomial.isIntegerPower()
                        ...
                        return SolveGeneralEquationUtils.solveZeroEquation(((BinaryOperation) factorizedPolynomial).getLeft(), var);
                    }
                } catch (SimplifyPolynomialUtils.PolynomialNotDecomposableException | EvaluationException e) {
                    // Nichts tun.
                }
            }
            ...
            if (degree == 1) {
                return SimplifyUtilities.union(zeros, PolynomialAlgebraUtils.solveLinearEquation(coefficients));
            }
            if (degree == 2) {
                return SimplifyUtilities.union(zeros, PolynomialAlgebraUtils.solveQuadraticEquation(coefficients));
            }
            if (degree == 3) {
                return SimplifyUtilities.union(zeros, PolynomialAlgebraUtils.solveCubicEquation(coefficients));
            }
            if (degree == 4) {
                return SimplifyUtilities.union(zeros, PolynomialAlgebraUtils.solveQuarticEquation(coefficients, var));
            }
    ...
    public static ExpressionCollection getRationalZerosOfRationalPolynomial(Expression f, String var) {
        ...
        try {
            coefficients = SimplifyPolynomialUtils.getPolynomialCoefficients(f, var);
        } catch (EvaluationException e) {
    ...
    public static ExpressionCollection findAllRationalZerosOfRationalPolynomial(ExpressionCollection coefficients, ExpressionCollection rationalZeros)
        ...
        for (int i = 0; i < coefficients.getBound() - 1; i++) {
            ...
            if (zero.length > 0) {
                ...
                coefficientsOfDivisionQuotient = SimplifyPolynomialUtils.polynomialDivision(coefficientsOfDivisionQuotient, divisor)[0];
            } else {
                break;
            }
        }
    ...
    public static ExpressionCollection solvePolynomialEquationWithFractionalExponents(Expression f, String var)
        ...
        if (!SimplifyPolynomialUtils.isPolynomial(f, var) && !PolynomialAlgebraUtils.isPolynomialInRationalPowers(f, var)) {
        ...
        if (m.compareTo(BigInteger.ONE) > 0) {
            ...
            ExpressionCollection zeros = solvePolynomialEquation(SimplifyPolynomialUtils.getPolynomialCoefficients(fSubstituted, var), var);
