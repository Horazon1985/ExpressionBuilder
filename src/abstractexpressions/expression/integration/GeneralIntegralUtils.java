...
public abstract class GeneralIntegralUtils {
    ...
    protected static Expression indefiniteIntegration(Operator expr, boolean prepareIntegrand) throws EvaluationException, NotAlgebraicallyIntegrableException {
        ...
        try {
            return SpecialIntegrationUtils.integrateQuotientOfLinearPolynomialAndQuadraticPolynomial(expr).simplifyBasic();
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Typ: (A*x + B) / (a*x^2 + b*x + c)^k, k > 1.
        try {
            return SpecialIntegrationUtils.integrateQuotientOfLinearPolynomialAndPowerOfQuadraticPolynomial(expr).simplifyBasic();
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Partialbruchzerlegung
        try {
            return SpecialIntegrationUtils.integrateRationalFunction(expr).simplifyBasic();
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integration von Polynomen in x, exp, sin und cos mit linearen Argumenten in var.
        try {
            return SpecialIntegrationUtils.integrateProductOfPolynomialAndPowerOfTrigonometricFunction(expr);
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integriert Polynome in sin(x) und cos(x).
        try {
            return SpecialIntegrationUtils.integratePolynomialInComplexExponentialFunctions(expr);
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integration von P(x)*(a*x^2 + b*x + c)^((2*n + 1)/2), P = Polynom.
        try {
            return SpecialIntegrationUtils.integrateProductOfPolynomialAndOddPowerOfSqrtOfQuadraticPolynomial(expr);
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integration von P(x)/(a*x^2 + b*x + c)^((2*n + 1)/2).
        try {
            return SpecialIntegrationUtils.integrateQuotientOfPolynomialAndOddPowerOfSqrtOfQuadraticPolynomial(expr);
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integration von Q(x, (a*x^2 + b*x + c)^(1/2)), Q(s, t) rationale Funktion in s, t.
        try {
            return SpecialIntegrationUtils.integrateRationalFunctionInVarAndSqrtOfQuadraticFunction(expr);
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integration von Q(x, g(x)), Q(s, t) = rationale Funktion in s, t, y = g(x) derart, dass x = g(y) eine rationale Funktion in y ist.
        try {
            return SpecialIntegrationUtils.integrateRationalFunctionInVarAndAnotherAlgebraicExpressionEquation(expr);
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integration von Q(x^(1/n)), Q = rationale Funktion.
        try {
            return SpecialIntegrationUtils.integrateRationalFunctionInRationalPowerOfVar(expr);
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integration von R(exp(a*x)), R(t) = rationale Funktion in t.
        try {
            return SpecialIntegrationUtils.integrateRationalFunctionInExp(expr).simplifyBasic();
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integragtion von R(cos(a*x), sin(a*x)), R(t) = rationale Funktion in t.
        try {
            return SpecialIntegrationUtils.integrateRationalFunctionInTrigonometricFunctions(expr).simplifyBasic();
        } catch (NotAlgebraicallyIntegrableException e) {
        }

        // Integragtion von P(x)*f(x)^n, P = Polynom, f = arcsin, arccos, arsinh, arcosh, n positive ganze Zahl.
        try {
            return SpecialIntegrationUtils.integrateProductOfPolynomialAndPowerOfArcusOrAreaFunction(expr).simplifyBasic();
        } catch (NotAlgebraicallyIntegrableException e) {
        }
        ...
        try {
            result = RischAlgorithmUtils.integrateByRischAlgorithmForTranscendentalExtension(expr);
            if (!result.containsIndefiniteIntegral()) {
                // Ergebnis nur DANN ausgeben, wenn darin keine weiteren Integrale vorkommen.
                return result;
            }
    ...
    private static Expression[] getSeparationForPartialIntegration(ExpressionCollection factorsNumerator, ExpressionCollection factorsDenominator, String var) throws NotAlgebraicallyIntegrableException {
        ...
        for (int i = 0; i < factorsNumerator.getBound(); i++) {
            ...
            if (SimplifyPolynomialUtils.isPolynomial(factorsNumerator.get(i), var)) {
                numberOfPolynomialFactorsInNumerator++;
                numberOfRationalFactorsInNumerator++;
                indexOfPolynomialFactorsInNumerator = i;
            } else if (SimplifyRationalFunctionUtils.isRationalFunction(factorsNumerator.get(i), var)) {
                numberOfRationalFactorsInNumerator++;
                    ...
                    && SimplifyPolynomialUtils.isLinearPolynomial(((Function) factorsNumerator.get(i)).getLeft(), var)) {
                ...
                numberOfExponentialOrTrigonometricalFactorsInNumerator++;
                    ...
                    && SimplifyPolynomialUtils.isLinearPolynomial(((Function) ((BinaryOperation) factorsNumerator.get(i)).getLeft()).getLeft(), var)) {
                numberOfExponentialOrTrigonometricalFactorsInNumerator++;
            } else if ((factorsNumerator.get(i).isFunction(TypeFunction.lg) || factorsNumerator.get(i).isFunction(TypeFunction.ln))
                    && SimplifyRationalFunctionUtils.isRationalFunction(((Function) factorsNumerator.get(i)).getLeft(), var)) {
                ...
                indexOfLogarithmicFactorInNumerator = i;
                    ...
                    && SimplifyRationalFunctionUtils.isRationalFunction(((Function) ((BinaryOperation) factorsNumerator.get(i)).getLeft()).getLeft(), var)) {
                numberOfLogarithmicFactorsInNumerator++;
                indexOfLogarithmicFactorInNumerator = i;
            }
        ...
        for (int i = 0; i < factorsDenominator.getBound(); i++) {
            ...
            if (SimplifyRationalFunctionUtils.isRationalFunction(factorsDenominator.get(i), var)) {
    ...
    private static Expression integrateByPartialIntegration(Operator expr) throws EvaluationException, NotAlgebraicallyIntegrableException {
         ...
         */
        if (SimplifyRationalFunctionUtils.isRationalFunction(uPrime, var)
                ...
                && SimplifyRationalFunctionUtils.isRationalFunction(((Function) v).getLeft(), var)
                && !SimplifyRationalFunctionUtils.isRationalFunction(u, var)) {
            throw new NotAlgebraicallyIntegrableException();
        }
        if (SimplifyRationalFunctionUtils.isRationalFunction(uPrime, var)
                ...
                && SimplifyRationalFunctionUtils.isRationalFunction(((Function) ((BinaryOperation) v).getLeft()).getLeft(), var)
                && !SimplifyRationalFunctionUtils.isRationalFunction(u, var)) {
            throw new NotAlgebraicallyIntegrableException();
        }
