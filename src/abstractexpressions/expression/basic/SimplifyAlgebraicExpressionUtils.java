...
public abstract class SimplifyAlgebraicExpressionUtils {
    ...
    public static Expression factorizeRationalFactorsFromRationalRoots(BinaryOperation expr) {
        ...
        if (a.compareTo(BigInteger.valueOf(ComputationBounds.BOUND_ARITHMETIC_DIVISORS_OF_INTEGERS)) <= 0) {
            ...
            setOfDivisors = ArithmeticUtils.getDivisors(a);
        ...
        if (b.compareTo(BigInteger.valueOf(ComputationBounds.BOUND_ARITHMETIC_DIVISORS_OF_INTEGERS)) <= 0) {
            ...
            setOfDivisors = ArithmeticUtils.getDivisors(b);
    ...
    public static Expression binomialExpansion(BinaryOperation expr, int exponent) {
        ...
        /*
         ...
         1 + k)!/[(n - 1)! * k!]
         */
        BigInteger numberOfSummandsInResult = ArithmeticUtils.factorial(n - 1 + exponent).divide(ArithmeticUtils.factorial(n - 1).multiply(ArithmeticUtils.factorial(exponent)));
        ...
        for (int i = 0; i < numberOfSummandsInResult.intValue(); i++) {
            ...
            binomialCoefficient = ArithmeticUtils.factorial(exponent);
            for (int j = 0; j < n; j++) {
                binomialCoefficient = binomialCoefficient.divide(ArithmeticUtils.factorial(exponents[j]));
            }
            ...
            if (factors.get(i) != null && factors.get(i).isPower()
                ...
                for (int j = i + 1; j < factors.getBound(); j++) {
                    ...
                    if (factors.get(j) != null && factors.get(j).isPower()
                        ...
                        commonRootDegree = ArithmeticUtils.lcm(m, n);

                        // In diesem Fall werden die Potenzen von simplify() nicht vollständig ausgerechnet.
                        if (ArithmeticUtils.lcm(m, n).divide(m).multiply(p).add(ArithmeticUtils.lcm(m, n).divide(n).multiply(q)).compareTo(BigInteger.valueOf(ComputationBounds.BOUND_ALGEBRA_MAX_DEGREE_OF_COMMON_ROOT)) > 0) {
                            continue;
                        }
                        ...
                        resultFactors.add(new Constant(a.pow(ArithmeticUtils.lcm(m, n).divide(m).multiply(p).intValue()).multiply(c.pow(ArithmeticUtils.lcm(m, n).divide(n).multiply(q).intValue()))).div(new Constant(b.pow(ArithmeticUtils.lcm(m, n).divide(m).multiply(p).intValue()).multiply(d.pow(ArithmeticUtils.lcm(m, n).divide(n).multiply(q).intValue())))).pow(BigInteger.ONE, commonRootDegree));
                        factors.remove(i);
                        factors.remove(j);
                        break;
    ...
    private static Expression root(Expression radicand, int n) throws RootNotRationalException {
        ...
        if (radicand.isIntegerConstant()) {
            ...
            try {
                sqrtOfNumerator = ArithmeticUtils.root(numerator, n);
                sqrtOfDenominator = ArithmeticUtils.root(denominator, n);
            } catch (EvaluationException e) {
                throw new RootNotRationalException();
            }
