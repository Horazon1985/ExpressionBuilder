...
public class GraphicPanelFormula extends JPanel {
    ...
    private int getLengthOfString(Graphics g, String s, int fontSize) {
        setFont(g, fontSize);
    ...
    private int getLengthOfCommand(Graphics g, Command c, int fontSize) {
        ...
        int resultLength = getLengthOfString(g, c.getName(), fontSize)
        ...
        switch (c.getTypeCommand()) {
            case def:
                if (params.length == 2) {
                    ...
                    resultLength = resultLength + getLengthOfString(g, (String) params[0], fontSize)
                    ...
                    resultLength = resultLength + getLengthOfString(g, (String) params[0], fontSize)
                            + 2 * getWidthOfBracket(fontSize) + (params.length - 3) * getLengthOfString(g, ", ", fontSize);

                    for (int i = 1; i < params.length - 1; i++) {
                        resultLength = resultLength + getLengthOfString(g, (String) params[i], fontSize);
                    }
            ...
            case latex:
                ...
                break;
            case solve:
                resultLength = resultLength + (params.length - 2) * getLengthOfString(g, ", ", fontSize);
                for (Object param : params) {
                    if (param instanceof Expression) {
                        resultLength = resultLength + getLengthOfExpression(g, (Expression) param, fontSize);
                    } else if (param instanceof Expression[]) {
                        ...
                        setFont(g, fontSize);
                        resultLength = resultLength + getLengthOfString(g, String.valueOf((Integer) param), fontSize);
                    } else if (param instanceof String) {
                        setFont(g, fontSize);
                        resultLength = resultLength + getLengthOfString(g, (String) param, fontSize);
                    }
                }
                break;
            case normal:
            case tangent:
                ...
                resultLength = resultLength + ((Map) params[1]).size() * (getLengthOfString(g, ", ", fontSize) + getWidthOfSignEquals(g, fontSize));
                ...
                Map<String, Expression> varsWithValues = (Map<String, Expression>) params[1];
                for (String var : varsWithValues.keySet()) {
                    setFont(g, fontSize);
                    // Pixellänge der jeweiligen Variable hinzuaddieren.
                    resultLength = resultLength + getLengthOfString(g, var, fontSize);
                    // Pixellänge des jeweiligen Variablenwertes hinzuaddieren.
                    resultLength = resultLength + getLengthOfExpression(g, (Expression) varsWithValues.get(var), fontSize);
                }
                break;
            default:
                // Sonstiger Fall.
                resultLength = resultLength + Math.max(params.length - 1, 0) * getLengthOfString(g, ", ", fontSize);
                for (Object param : params) {
                    if (param instanceof Expression) {
                        resultLength = resultLength + getLengthOfExpression(g, (Expression) param, fontSize);
                    } else if (param instanceof Expression[]) {
    ...
    private int getLengthOfLogicalConstant(Graphics g, LogicalConstant logExpr, int fontSize) {
        return getLengthOfString(g, logExpr.toString(), fontSize);
    }
    ...
    private int getLengthOfLogicalBinaryOperation(Graphics g, LogicalBinaryOperation logExpr, int fontSize) {
        ...
        if (logExpr.isAnd()) {
            ...
            if (logExpr.getLeft().isOr() || logExpr.getLeft().isImpl() || logExpr.getLeft().isEquiv()) {
                lengthLeft = getLengthOfLogicalExpression(g, logExpr.getLeft(), fontSize)
                        + 2 * getWidthOfBracket(fontSize);
            } else {
                lengthLeft = getLengthOfLogicalExpression(g, logExpr.getLeft(), fontSize);
            }
            if (logExpr.getRight().isOr() || logExpr.getRight().isImpl() || logExpr.getRight().isEquiv()) {
                lengthRight = getLengthOfLogicalExpression(g, logExpr.getRight(), fontSize)
                        + 2 * getWidthOfBracket(fontSize);
            } else {
                lengthRight = getLengthOfLogicalExpression(g, logExpr.getRight(), fontSize);
            }
            return lengthLeft + getWidthOfSignOr(fontSize) + lengthRight;
            ...
            return lengthLeft + getWidthOfSignImplication(fontSize) + lengthRight;
    ...
    private int getLengthOfMatrixExpression(Graphics g, MatrixExpression matExpr, int fontSize) {
        ...
        if (matExpr instanceof Matrix) {
            ...
            if (((MatrixFunction) matExpr).getLeft() instanceof Matrix
                ...
                return getLengthOfString(g, ((MatrixFunction) matExpr).getName(), fontSize)
                        + getLengthOfMatrixExpression(g, ((MatrixFunction) matExpr).getLeft(), fontSize);
            } else {
                return getLengthOfString(g, ((MatrixFunction) matExpr).getName(), fontSize) + 2 * getWidthOfBracket(fontSize)
                        + getLengthOfMatrixExpression(g, ((MatrixFunction) matExpr).getLeft(), fontSize);
            }
        ...
        switch (matOperator.getType()) {
            ...
            case diff: {
                ...
                if (params.length == 2) {
                    ...
                    result = getWidthOfSignPartial(g, fontSize) + getLengthOfString(g, (String) params[1], fontSize)
                            + 2 * getWidthOfBracket(fontSize) + getLengthOfMatrixExpression(g, (MatrixExpression) params[0], fontSize);

                } else if (params.length == 3 && params[2] instanceof Integer) {

                    int widthOfDifferentialOperator = getWidthOfSignPartial(g, fontSize) + getLengthOfString(g, (String) params[1], fontSize);
                    setFont(g, getSizeForSup(fontSize));
                    widthOfDifferentialOperator = widthOfDifferentialOperator + getLengthOfString(g, String.valueOf((int) params[2]), fontSize);
                    ...
                    int widthOfDifferentialOperator = getLengthOfString(g, (String) params[1], fontSize);
                    for (int i = 2; i < params.length; i++) {
                        widthOfDifferentialOperator = widthOfDifferentialOperator + getLengthOfString(g, (String) params[i], fontSize);
                    }
            ...
            case integral: {
                ...
                /*
                 ...
                 Pixel).
                 */
                return result + getLengthOfString(g, "d" + (String) params[1], fontSize) + fontSize / 2;
            ...
            case laplace:
                ...
                setFont(g, fontSize);
                result += (params.length - 1) * getLengthOfString(g, ", ", fontSize);
                for (int i = 1; i < params.length; i++) {
                    result = result + getLengthOfString(g, (String) params[i], fontSize);
                }
                return result;
            ...
            case prod: {
                ...
                setFont(g, getSizeForSup(fontSize));
                int lengthIndexVar = getLengthOfString(g, (String) params[1] + " = ", fontSize);
            ...
            default: {
                ...
                result = getLengthOfString(g, matOperator.getType().getOperatorName(), fontSize) + 2 * getWidthOfBracket(fontSize);
                ...
                for (int i = 0; i < params.length; i++) {
                    ...
                    if (params[i] instanceof Expression) {
                        ...
                        result = result + getLengthOfString(g, String.valueOf((Integer) params[i]), fontSize);
                    }

                    if (i < params.length - 1) {
                        result = result + getLengthOfString(g, ", ", fontSize);
                    }
    ...
    private int getHeightOfExpression(Graphics g, Expression expr, int fontSize) {
        ...
        if (expr instanceof Constant) {
            ...
            SelfDefinedFunction exprAsSelfdefinedfunction = (SelfDefinedFunction) expr;
            Expression[] left = exprAsSelfdefinedfunction.getLeft();
    ...
    private int getLengthOfConstant(Graphics g, Constant expr, int fontSize) {
        ...
        if (expr.getValue().compareTo(BigDecimal.ZERO) < 0) {
            return getWidthOfSignMinus(g, fontSize) + getLengthOfString(g, s.substring(1, s.length()), fontSize);
        }
        return getLengthOfString(g, s, fontSize);
    }
    ...
    private int getLengthOfVariable(Graphics g, Variable expr, int fontSize) {
        ...
        int lengthVar = getLengthOfString(g, varName, fontSize);
        int lengthApostrophs = getLengthOfString(g, apostrophs, fontSize);
        int lengthIndex = getLengthOfString(g, index, getSizeForSub(fontSize));
    ...
    private int getLengthOfMultiIndexVariable(Graphics g, MultiIndexVariable v, int fontSize) {
        ...
        int lengthOfVarWithoutIndices = getLengthOfString(g, name, fontSize);

        // Länge Variablennamens zusammen mit den Apostrophs berechnen.
        if (name.contains("'")) {
            lengthOfVarWithoutIndices += getLengthOfString(g, name.substring(name.indexOf("'")), fontSize);
        }
        ...
        int lengthOfVarWithoutPrimes = getLengthOfString(g, nameWithoutPrimes, fontSize) + fontSize / 3;
        for (int i = 0; i < v.getIndices().size(); i++) {
            lengthOfVarWithoutPrimes += getLengthOfString(g, v.getIndices().get(i).toString(), getSizeForSub(fontSize));
            if (i < v.getIndices().size() - 1) {
                lengthOfVarWithoutPrimes += getLengthOfString(g, ",", getSizeForSub(fontSize));
            }
        }
    ...
    private int getLengthOfFunction(Graphics g, Function expr, int fontSize) {
        ...
        if (!expr.getType().equals(TypeFunction.abs)) {
            return getLengthOfString(g, expr.getName(), fontSize)
        ...
        switch (expr.getType()) {
            case diff: {
                ...
                if (params.length == 2) {
                    ...
                    int widthOfDifferentialOperator = getWidthOfSignPartial(g, fontSize) + getLengthOfString(g, (String) params[1], fontSize);
                    widthOfDifferentialOperator = widthOfDifferentialOperator + getLengthOfString(g, String.valueOf((int) params[2]), getSizeForSup(fontSize));
            ...
            case prod: {
                ...
                result = getWidthOfSignPi(g, fontSize, heightFactor);
                int lengthIndexVar = getLengthOfString(g, (String) params[1] + " = ", getSizeForSup(fontSize));
                result = Math.max(result, lengthIndexVar + getLengthOfExpression(g, (Expression) params[2], getSizeForSup(fontSize)));
                result = Math.max(result, getLengthOfExpression(g, (Expression) params[3], getSizeForSup(fontSize)));
            ...
            default: {
                ...
                switch (expr.getType()) {
                    ...
                    case taylor:
                        ...
                        break;
                    default:
                        lengthOfName = getLengthOfString(g, expr.getType().getOperatorName(), fontSize);
                        break;
                }
    ...
    private int getLengthOfSelfDefinedFunction(Graphics g, SelfDefinedFunction expr, int fontSize) {
        ...
        int result = getLengthOfString(g, expr.getName(), fontSize);
        result = result + 2 * getWidthOfBracket(fontSize)
                + (expr.getArguments().length - 1) * getLengthOfString(g, ", ", fontSize);
    ...
    private int getLengthOfOutput(Graphics g, int fontSize, Object... output) {
        ...
        for (Object out : output) {
            ...
            if (out instanceof EditableAbstractExpression) {
                ...
                String text = ((EditableString) out).getText();
                length = length + getLengthOfString(g, text, fontSize);
                ...
                length = length + getLengthOfString(g, (String) out, fontSize);
    ...
    private int getWidthOfSignPartial(Graphics g, int fontSize) {
        int result = getLengthOfString(g, "\u2202", fontSize);
        return result;
    }
    ...
    private void drawSignEquals(Graphics g, int x_0, int y_0, int fontSize) {
        g.drawString("=", x_0 + (getWidthOfSignEquals(g, fontSize) - getLengthOfString(g, "=", fontSize)) / 2, y_0);
    }
    ...
    private void drawMultiIndexVariable(Graphics g, MultiIndexVariable v, int x_0, int y_0, int fontSize) {
        ...
        String name = v.getName();
        String nameWithoutPrimes = name.replaceAll("'", "");
        int varWithoutPrimesWidth = getLengthOfString(g, nameWithoutPrimes, fontSize);
        ...
        for (int i = 0; i < v.getIndices().size(); i++) {
            ...
            distanceFromVarWithoutPrimes += getLengthOfString(g, v.getIndices().get(i).toString(), getSizeForSub(fontSize));
            if (i < v.getIndices().size() - 1) {
                g.drawString(",", x_0 + varWithoutPrimesWidth + distanceFromVarWithoutPrimes, y_0 + Math.max(getSizeForSub(fontSize) / 2, 1));
                distanceFromVarWithoutPrimes += getLengthOfString(g, ",", getSizeForSub(fontSize));
            }
        }
    ...
    private void drawFunction(Graphics g, Function f, int x_0, int y_0, int fontSize) {
        ...
        if (!f.getType().equals(TypeFunction.abs)) {
            ...
            drawOpeningBracket(g, x_0 + getLengthOfString(g, f.getName(), fontSize), y_0, fontSize, heightLeft);
            drawExpression(g, f.getLeft(),
                    x_0 + getLengthOfString(g, f.getName(), fontSize) + getWidthOfBracket(fontSize),
                    y_0, fontSize);
            setFont(g, fontSize);
    ...
    private void drawOperatorDiff(Graphics g, Operator operator, int x_0, int y_0, int fontSize) {
        ...
        if (params.length == 2) {
            ...
            lengthDifferentialOperator = getWidthOfSignPartial(g, fontSize) + getLengthOfString(g, (String) params[1], fontSize);
            ...
            lengthDifferentialOperator = lengthPartialSign + getLengthOfString(g, (String) params[1], fontSize) + getLengthOfConstant(g, new Constant((int) params[2]), getSizeForSup(fontSize));
            ...
            drawExpression(g, new Constant((int) params[2]), x_0 + lengthPartialSign + getLengthOfString(g, (String) params[1], fontSize),
            ...
            int lengthPartialSign = getWidthOfSignPartial(g, fontSize);
            lengthDifferentialOperator = getLengthOfString(g, (String) params[1], fontSize);
            for (int i = 2; i < params.length; i++) {
                lengthDifferentialOperator = lengthDifferentialOperator + getLengthOfString(g, (String) params[i], fontSize);
            }
            ...
            for (int i = 1; i < params.length; i++) {
                ...
                l_current = l_current + lengthPartialSign + getLengthOfString(g, (String) params[i], fontSize);
            }
    ...
    private void drawOperatorInt(Graphics g, Operator operator, int x_0, int y_0, int fontSize) {
        ...
        if (params.length == 2) {
            ...
            drawVariable(g, Variable.create((String) params[1]),
                    x_0 + getWidthOfSignIntegral(g, fontSize, heightIntegrand) + lengthIntegrand + getLengthOfString(g, " d", fontSize),
            ...
            drawVariable(g, Variable.create((String) params[1]),
                    x_0 + lengthOfIntegralSignWithLimits + lengthIntegrand + getLengthOfString(g, " d", fontSize),
    ...
    private void drawOperatorProd(Graphics g, Operator operator, int x_0, int y_0, int fontSize) {
        ...
        int lengthEquals = getLengthOfString(g, " = ", getSizeForSub(fontSize));
        int lengthLowerLimit = getLengthOfExpression(g, (Expression) params[2], getSizeForSup(fontSize));
        int lengthUpperLimit = getLengthOfExpression(g, (Expression) params[3], getSizeForSup(fontSize));
    ...
    private void drawOperatorDefault(Graphics g, Operator operator, int x_0, int y_0, int fontSize) {
        ...
        switch (operator.getType()) {
            ...
            default:
                ...
                distanceFromBeginningOfOperator += getLengthOfString(g, name, fontSize);
                break;
        }
        ...
        for (int i = 0; i < left.length; i++) {
            ...
            if (left[i] instanceof Expression) {
                ...
                distanceFromBeginningOfOperator = distanceFromBeginningOfOperator + getLengthOfString(g, (String) left[i], fontSize);
            } else if (left[i] instanceof Integer) {
                setFont(g, fontSize);
                g.drawString(String.valueOf((Integer) left[i]), x_0 + distanceFromBeginningOfOperator,
                        y_0 - (heightCenterOperator - (2 * fontSize) / 5));
                distanceFromBeginningOfOperator = distanceFromBeginningOfOperator + getLengthOfString(g, String.valueOf((Integer) left[i]), fontSize);
            }
            ...
            if (i < left.length - 1) {
                ...
                g.drawString(", ", x_0 + distanceFromBeginningOfOperator,
                        y_0 - (heightCenterOperator - (2 * fontSize) / 5));
                distanceFromBeginningOfOperator = distanceFromBeginningOfOperator + getLengthOfString(g, ", ", fontSize);
            }
    ...
    private void drawSelfDefinedFunction(Graphics g, SelfDefinedFunction f, int x_0, int y_0, int fontSize) {
        ...
        drawOpeningBracket(g, x_0 + getLengthOfString(g, ((SelfDefinedFunction) f).getName(), fontSize), y_0, fontSize, heightF);
        ...
        for (int i = 0; i < left.length; i++) {
            ...
            drawExpression(g, left[i],
                    x_0 + getLengthOfString(g, ((SelfDefinedFunction) f).getName(), fontSize)
            ...
            if (i < left.length - 1) {
                setFont(g, fontSize);
                g.drawString(", ", x_0 + getLengthOfString(g, ((SelfDefinedFunction) f).getName(), fontSize)
                        + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                        y_0 - (heightCenterF - (2 * fontSize) / 5));
                distanceFromOpeningBracket = distanceFromOpeningBracket + getLengthOfString(g, ", ", fontSize);
            }
    ...
    private void drawMatrixFunction(Graphics g, MatrixFunction matExpr, int x_0, int y_0, int fontSize) {
        ...
        int heightLeft = getHeightOfMatrixExpression(g, matExpr.getLeft(), fontSize);
        int heightCenterLeft = getHeightOfCenterOfMatrixExpression(g, matExpr.getLeft(), fontSize);
        int lengthName = getLengthOfString(g, matExpr.getName(), fontSize);
    ...
    private void drawMatrixOperatorLaplace(Graphics g, MatrixOperator matOperator, int x_0, int y_0, int fontSize) {
        ...
        for (int i = 1; i < matOperator.getParams().length; i++) {
            ...
            distanceFromOpeningBracket += getLengthOfString(g, ", ", fontSize);
            ...
            distanceFromOpeningBracket += getLengthOfString(g, (String) matOperator.getParams()[i], fontSize);
        }
        // )
    ...
    private void drawMatrixOperatorProd(Graphics g, MatrixOperator matOperator, int x_0, int y_0, int fontSize) {
        ...
        int heightCenterLowerLimit = getHeightOfCenterOfExpression(g, (Expression) params[2], getSizeForSup(fontSize));
        int heightCenterVarEquals = getHeightOfCenterOfExpression(g, Variable.create((String) params[1]), getSizeForSup(fontSize));
        int lengthVarEquals = getLengthOfString(g, (String) params[1] + " = ", getSizeForSup(fontSize));
        int lengthLowerLimit = getLengthOfExpression(g, (Expression) params[2], getSizeForSup(fontSize));
        int lengthUpperLimit = getLengthOfExpression(g, (Expression) params[3], getSizeForSup(fontSize));
        int lengthPiSign = getWidthOfSignPi(g, fontSize, heightFactor);
    ...
    private void drawMatrixOperatorDefault(Graphics g, MatrixOperator matOperator, int x_0, int y_0, int fontSize) {
        ...
        g.drawString(name, x_0, y_0 - (heightCenterOperator - (2 * fontSize) / 5));
        drawOpeningBracket(g, x_0 + getLengthOfString(g, name, fontSize), y_0, fontSize, heightOperator);
        ...
        for (int i = 0; i < left.length; i++) {
            ...
            if (left[i] instanceof Expression) {
                drawExpression(g, (Expression) left[i],
                        x_0 + getLengthOfString(g, name, fontSize)
                        + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                        y_0 - (heightCenterOperator - getHeightOfCenterOfExpression(g, (Expression) left[i], fontSize)), fontSize);
                ...
                setFont(g, fontSize);
                g.drawString((String) left[i], x_0 + getLengthOfString(g, name, fontSize)
                        + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                        y_0 - (heightCenterOperator - (2 * fontSize) / 5));
                distanceFromOpeningBracket = distanceFromOpeningBracket + getLengthOfString(g, (String) left[i], fontSize);
            } else if (left[i] instanceof Integer) {
                setFont(g, fontSize);
                g.drawString(String.valueOf((Integer) left[i]), x_0 + getLengthOfString(g, name, fontSize)
                        + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                        y_0 - (heightCenterOperator - (2 * fontSize) / 5));
                distanceFromOpeningBracket = distanceFromOpeningBracket + getLengthOfString(g, String.valueOf((Integer) left[i]), fontSize);
            }

            if (i < left.length - 1) {
                setFont(g, fontSize);
                g.drawString(", ", x_0 + getLengthOfString(g, name, fontSize)
                        + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                        y_0 - (heightCenterOperator - (2 * fontSize) / 5));
                distanceFromOpeningBracket = distanceFromOpeningBracket + getLengthOfString(g, ", ", fontSize);
            }
    ...
    private void drawCommandDef(Graphics g, Command c, int x_0, int y_0, int fontSize) {
        ...
        int lengthName = getLengthOfString(g, c.getName(), fontSize);
        ...
        if (params.length == 2) {
            ...
            // Fall: Variablendeklaration.
            int lengthVar = getLengthOfString(g, (String) params[0], fontSize);
            ...
            g.drawString(functionName, x_0 + lengthName + getWidthOfBracket(fontSize),
                    y_0 - (heightCenterCommand - (2 * fontSize) / 5));
            distanceFromOpeningBracket = distanceFromOpeningBracket + getLengthOfString(g, functionName, fontSize);
    ...
    private void drawCommandsNormalOrTangent(Graphics g, Command c, int x_0, int y_0, int fontSize) {
        ...
        for (Iterator iter = varsWithValues.keySet().iterator(); iter.hasNext();) {
            ...
            g.drawString(varCurrent, x_0 + lengthName
                    + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                    y_0 - (heightCenterCommand - (2 * fontSize) / 5));
            distanceFromOpeningBracket = distanceFromOpeningBracket + getLengthOfString(g, varCurrent, fontSize);
            // "="-Zeichen zeichnen.
            drawSignEquals(g, x_0 + lengthName
                    + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                    y_0 - (heightCenterCommand - (2 * fontSize) / 5), fontSize);
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            if (params[i] instanceof Expression) {
                ...
                g.drawString((String) params[i], x_0 + lengthName
                        + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                        y_0 - (heightCenterCommand - (2 * fontSize) / 5));
                distanceFromOpeningBracket = distanceFromOpeningBracket + getLengthOfString(g, (String) params[i], fontSize);
                ...
                g.drawString(String.valueOf((Integer) params[i]), x_0 + lengthName
                        + getWidthOfBracket(fontSize) + distanceFromOpeningBracket,
                        y_0 - (heightCenterCommand - (2 * fontSize) / 5));
                distanceFromOpeningBracket = distanceFromOpeningBracket + getLengthOfString(g, String.valueOf((Integer) params[i]), fontSize);
    ...
    private void drawOutput(Graphics g, int x_0, int y_0, int fontSize, Object... output) {
        ...
        for (Object out : output) {
            ...
            if (out instanceof Expression) {
                ...
                distanceFromBeginningOfOutput = distanceFromBeginningOfOutput + getLengthOfString(g, (String) out, fontSize);
                ...
                distanceFromBeginningOfOutput = distanceFromBeginningOfOutput + getLengthOfString(g, text, fontSize);
